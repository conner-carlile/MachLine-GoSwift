...
def pressures(speed_of_sound, p_inf):
    c = speed_of_sound
    ...
    with open('studies/Goswift/results/off_body_velocity.csv','r') as file:
        ...
        for row in rows:    
            #p.append( (1-(float(row['V'])/100)**2) ) #### Compressible Cp
            p.append((p_inf/(((1 + (gamma-1)/2 * (float(row['V'])/c)**2)**(gamma/(gamma-1)))))) #### local compressible pressure
            #p.append((((p_inf/(((1 + (gamma-1)/2 * (float(row['V'])/c)**2)**(gamma/(gamma-1))))))-p_inf)/p_inf)  #### dp crompressible
...
if __name__ == '__main__':
    ...
    #body_mesh = mesh.Mesh.from_file('studies/Goswift/meshes/SH_100_30.stl')
    ...
    body_mesh = mesh.Mesh.from_file('studies/Goswift/meshes/jaxxxa.stl')
    ...
    pressures(968.08, .00036392 ) #### run at 50000 ft
